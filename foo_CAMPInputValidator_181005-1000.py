...
def validateColEntries(colNumber, rowNumber, validEntriesLst):
...
def checkColumnData(colName, colNumber):
    ...
    for n in range(2,max_rows+1):
        if (validateColEntries(colNumber,n,valColEntries[colNumber])):
            pass
        else:
            allValidData = False
...
for c in colNumToCheck:
    checkColumnData(colHeadings[c], c)
